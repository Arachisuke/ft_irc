...
void Server::successfullJoin(int i)
{
    ...
    if (this->_channeList[i]->getTopic() != "")
    {
        ...
        send(this->_clientList[this->_nbrclient]->getFd(), topic.c_str(), topic.size(), MSG_DONTWAIT);
    }
    ...
    else
    {
        ...
        for (size_t j = 0; j < list.size(); j++)
        {
            if (!checkChannelNorm(list[j]))
                return (reply(476, this->_cmd[0], "Bad Channel Mask", *this->_clientList[this->_nbrclient]), (void)0);
            ...
            if (findChannel(list[j]) != -1)
            {
                ...
                if (this->_channeList[i]->isModeActif('i') && !_channeList[i]->isInvited(_clientList[_nbrclient]))
                    return (reply(473, list[j], "Cannot join channel (+i)", *this->_clientList[this->_nbrclient]), (void)0);

                if (this->_channeList[i]->isModeActif('k') && (_cmd.size() < 3 || _cmd[2] != this->_channeList[i]->getPassword()))
                    return (reply(475, list[j], "Cannot join channel (+k)", *this->_clientList[this->_nbrclient]), (void)0);

                if (this->_channeList[i]->isModeActif('l') && this->_channeList[i]->getUsers().size() >= this->_channeList[i]->getMaxUsers())
                    return (reply(471, list[j], "Cannot join channel (+l)", *this->_clientList[this->_nbrclient]), (void)0);
