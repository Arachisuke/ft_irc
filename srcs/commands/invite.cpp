...
void Server::invite()
{
    ...
    if (!channel->checkChannelNorm(_cmd[2])) // error  invalid channel name
    ...
    if (channel->getModes('i') && !channel->isOperator(_clientList[_nbrclient])) // error invite only et non operator
